...
class TestApi(unittest.TestCase):
    ...
    # Prueba de Integracion Traduccion
    def test_api_translatetodo(self):
        print('---------------------------------------')
        print('Starting - integration test Translate TODO')
        url = BASE_URL+"/todos"
        data = {
         "text": "Texto ejemplo para test de integración - TRANSLATE DM"
        }
        # Creo el registro
        response = requests.post(url, data=json.dumps(data))
        json_response = response.json()
        ...
        self.assertEqual(
            jsonbody['text'], "Texto ejemplo para test de integración - TRANSLATE DM", "Error en la petición API a {url}"
        )
        #Test GET TODO Traslate
        ...
        #Test GET TODO
        url = BASE_URL+"/todos/"+ID_TODO+"/de"
        response = requests.get(url)
        json_response = response.json()
        print('Response Translate Todo "de": '+ str(json_response))
        self.assertEqual(
            response.status_code, 200, "Error en la petición API a {url}"
        )
        print('responseTranslate: '+ str(json_response['text']))
